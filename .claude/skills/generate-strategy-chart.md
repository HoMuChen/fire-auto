...
用 Bash 執行以下 Python 腳本。根據需求調整 `WATCHLIST` 清單：
...
# 單檔：只放一組
WATCHLIST = [
    # (stock_id, stock_name, strategy_name)
    ("5225", "東科-KY", "雙底反彈"),
    ("7516", "清淨海", "威廉%R(14)"),
    ("6621", "華宇藥", "ATR通道回歸"),
...
# ─── HTML 模板 ───
# 使用 Lightweight Charts v4.1.0 (TradingView)，暗色主題
# 配色：背景 #0a0e17, 圖表區 #0f1320
#   收盤價 #3b82f6(藍), 買入 #22c55e(綠), 賣出 #ef4444(紅), 停損 #f59e0b(黃)
#   資金曲線 #22c55e(綠漸層)
# 頁面結構：導航列 → 標題 → 統計列 → 股價圖(420px) → 資金曲線(260px) → 交易明細表
...
新增策略時，在 `get_overlays()` 中加入對應分支。

## 注意事項

- 策略名稱必須與 `backtest.py` 的 `STRATEGIES` dict key 完全一致（含括號、斜線等特殊字元）
- 股價資料來自 `data/stock_prices/{stock_id}.csv`
